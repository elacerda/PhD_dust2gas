...
def plotBPT(ax, O3Hb, N2Ha, z = None, cmap = 'viridis', mask = None, labels = True, N = False):
    if mask is None: mask = np.zeros_like(O3Hb, dtype = np.bool_)
    from CALIFAUtils.lines import Lines
    plt.axis([-1.5, 1, -1.5, 1.5])
    if z is None:
        O3Hb, N2Ha = ma_mask_xyz(O3Hb, N2Ha, mask = mask) 
        sc = ax.scatter(N2Ha, O3Hb, marker = 'o', s = 30, edgecolor = 'none')#, alpha = 0.6)
    else:
        O3Hb, N2Ha, z = ma_mask_xyz(O3Hb, N2Ha, z, mask = mask)
        sc = ax.scatter(N2Ha, O3Hb, c = z, cmap = cmap, marker = 'o', s = 30, edgecolor = 'none')#, alpha = 0.6)
...
ax = plotBPT(ax, O3Hb__g, N2Ha__g, labels = False, N = 226176)
ax.yaxis.set_major_locator(MaxNLocator(4))
plt.setp(ax.get_xticklabels(), visible = False)

ax = plt.subplot2grid(grid_shape, (0, 1))
mask = (flag_residual > 0) 
ax = plotBPT(ax, O3Hb__g, N2Ha__g, mask = mask, labels = False)
plt.setp(ax.get_xticklabels(), visible = False)
...
ax = plotBPT(ax, O3Hb__g, N2Ha__g, mask = mask)
...
ax = plotBPT(ax, O3Hb__g, N2Ha__g, mask = mask, labels = False, N = 16479)
ax.xaxis.set_major_locator(MaxNLocator(4))
plt.setp(ax.get_yticklabels(), visible = False)
