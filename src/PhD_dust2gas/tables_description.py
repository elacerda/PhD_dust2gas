...
class zone(tbl.IsDescription):
    ...
    v_0 = tbl.Float64Col(pos=11, dflt=np.nan)
    v_d = tbl.Float64Col(pos=12, dflt=np.nan)

    # Neb
    tau_V_neb = tbl.Float64Col(pos=13, dflt=np.nan)
    etau_V_neb = tbl.Float64Col(pos=14, dflt=np.nan)
    F_obs_Hb = tbl.Float64Col(pos=15, dflt=np.nan)
    F_obs_O3 = tbl.Float64Col(pos=16, dflt=np.nan)
    F_obs_Ha = tbl.Float64Col(pos=17, dflt=np.nan)
    F_obs_N2 = tbl.Float64Col(pos=18, dflt=np.nan)
    eF_obs_Hb = tbl.Float64Col(pos=19, dflt=np.nan)
    eF_obs_O3 = tbl.Float64Col(pos=20, dflt=np.nan)
    eF_obs_Ha = tbl.Float64Col(pos=21, dflt=np.nan)
    eF_obs_N2 = tbl.Float64Col(pos=22, dflt=np.nan)
    baseline_Hb = tbl.Float64Col(pos=23, dflt=np.nan)
    baseline_O3 = tbl.Float64Col(pos=24, dflt=np.nan)
    baseline_Ha = tbl.Float64Col(pos=25, dflt=np.nan)
    baseline_N2 = tbl.Float64Col(pos=26, dflt=np.nan)
    EW_Hb = tbl.Float64Col(pos=27, dflt=np.nan)
    EW_O3 = tbl.Float64Col(pos=28, dflt=np.nan)
    EW_Ha = tbl.Float64Col(pos=29, dflt=np.nan)
    EW_N2 = tbl.Float64Col(pos=30, dflt=np.nan)
    sigma_Hb = tbl.Float64Col(pos=31, dflt=np.nan)
    sigma_O3 = tbl.Float64Col(pos=32, dflt=np.nan)
    sigma_Ha = tbl.Float64Col(pos=33, dflt=np.nan)
    sigma_N2 = tbl.Float64Col(pos=34, dflt=np.nan)
    esigma_Hb = tbl.Float64Col(pos=35, dflt=np.nan)
    esigma_O3 = tbl.Float64Col(pos=36, dflt=np.nan)
    esigma_Ha = tbl.Float64Col(pos=37, dflt=np.nan)
    esigma_N2 = tbl.Float64Col(pos=38, dflt=np.nan)
    pos_O3 = tbl.Float64Col(pos=39, dflt=np.nan)
    pos_Hb = tbl.Float64Col(pos=40, dflt=np.nan)
    pos_Ha = tbl.Float64Col(pos=41, dflt=np.nan)
    pos_N2 = tbl.Float64Col(pos=42, dflt=np.nan)
    epos_Hb = tbl.Float64Col(pos=43, dflt=np.nan)
    epos_O3 = tbl.Float64Col(pos=44, dflt=np.nan)
    epos_Ha = tbl.Float64Col(pos=45, dflt=np.nan)
    epos_N2 = tbl.Float64Col(pos=46, dflt=np.nan)
    logOH = tbl.Float64Col(pos=47, dflt=np.nan)

    flag_RGB = tbl.UInt8Col(pos=48, dflt=0)
    flag_residual = tbl.UInt8Col(pos=49, dflt=0)
